...
def get_word_score(word, n):
    ...
    for i in word.lower():
        summ += SCRABBLE_LETTER_VALUES[i]
    return summ * max(1, 7 * word_length - 3 * (n - word_length))
#
...
def is_valid_word(word, hand, word_list):
    ...
    for v in VOWELS:
        new_hand = hand.copy()
        new_hand[v] = new_hand.get(v, 0) + 1
        if is_valid_word(word.replace('*', v), new_hand, word_list):
            return True
    return False
#
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while calculate_handlen(hand) > 0:
        ...
        display_hand(hand)
        # Ask user for input
        s = input("Enter word, or “!!” to indicate that you are finished: ")
        ...
        # Otherwise (the input is not two exclamation points):
            ...
            if is_valid_word(s, hand, word_list):
                # Tell the user how many points the word earned,
                # and the updated total score
                score = get_word_score(s, calculate_handlen(hand))
                total_score += score
                print(f"“{s}” earned {score} points. Total: {total_score} points")
            # Otherwise (the word is not valid):
            else:
                # Reject invalid word (print a message)
                print("This is not a valid word. Please choose another word.")
            # update the user's hand by removing the letters of their inputted word
            hand = update_hand(hand, s)
        print()
...
def substitute_hand(hand, letter):
    ...
    for i in hand.keys():
        possible_letters = possible_letters.replace(i, '')
    ...
    while total_hands > 0:
        if not repeat:
            ...
            if s == "yes":
                ...
                print("Seems like you answered no :)")
            print()
